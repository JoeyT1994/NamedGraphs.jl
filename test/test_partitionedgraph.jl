...
using Graphs: a_star, center, connected_components, diameter, edges, has_vertex,
    is_connected, is_directed, is_tree, ne, neighbors, nv, radius, random_regular_graph,
    rem_vertex!, vertices
using Metis: Metis
using NamedGraphs.GraphsExtensions: add_edges!, add_vertices!, boundary_edges,
    default_root_vertex, forest_cover, is_path_graph, is_self_loop, spanning_forest,
    spanning_tree, subgraph, vertextype
...
using NamedGraphs.PartitionedGraphs: PartitionEdge, PartitionVertex, PartitionedGraph,
    PartitionsGraphView, boundary_partitionedges, partitionedge, partitionedges,
    partitions_graph, partitionvertex, partitionvertices, unpartitioned_graph
...
@testset "Test Partitioned Graph Partition Edge and Vertex Finding" begin
    ...
    boundary_sizes =
        [length(boundary_partitionedges(pg, pv)) for pv in partitionvertices(pg)]
